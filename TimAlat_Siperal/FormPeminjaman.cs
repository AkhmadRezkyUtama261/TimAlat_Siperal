    ...
    public partial class FormPeminjaman : Form
    {
        ...
        private void btnCari_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = konn.GetConn())
            {
                try
                {
                    ...
                    else { MessageBox.Show("Warga tidak ditemukan!"); }
                }
        ...
        private void cbAlat_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = konn.GetConn())
            {
                ...
                catch { lblStok.Text = "0"; }
            }
        }

        private void btnSimpan_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(txtJumlah.Text)) return;
            using (SqlConnection conn = konn.GetConn())
            {
                try
                {
                    ...
                    cmd = new SqlCommand("INSERT INTO Peminjaman (petugasID, NIK, alatID, Status, Jumlah_Pinjam, Tanggal_Pinjam) VALUES (1, @nik, @id, 'DIPINJAM', @jml, GETDATE())", conn);
                    cmd.Parameters.AddWithValue("@nik", txtNIK.Text.Trim());
        ...
        {
            if (e.RowIndex >= 0) idTerpilih = dgvPeminjaman.Rows[e.RowIndex].Cells["ID"].Value.ToString();
        }

        private void btnKembalikan_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(idTerpilih)) return;
            using (SqlConnection conn = konn.GetConn())
            {
                try
                {
                    conn.Open();
                    cmd = new SqlCommand("SELECT alatID, Jumlah_Pinjam FROM Peminjaman WHERE peminjamanID = @id", conn);
